...
export function validateState(value: unknown): UiState {
  ...
  for (const [name, entry] of Object.entries(value.components)) {
    if (!record(entry) || typeof entry.version !== 'string' || typeof entry.path !== 'string' || (entry.repository !== undefined && typeof entry.repository !== 'string')) throw new Error(`ui.json component "${name}" must contain string "version" and "path".`);
    ...
    components[name] = { version: entry.version, path: entry.path, ...(typeof entry.repository === 'string' ? { repository: entry.repository } : {}), ...(entry.files ? { files: entry.files as ComponentState['files'] } : {}), ...(Array.isArray(entry.dependencies) ? { dependencies: entry.dependencies as ComponentState['dependencies'] } : {}) };
  }
