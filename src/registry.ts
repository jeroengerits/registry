...
export function validateComponentManifest(value: unknown): ComponentManifest {
  if (!record(value) || value.schemaVersion !== 1 || typeof value.name !== 'string' || !/^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/.test(value.name) || (value.description !== undefined && typeof value.description !== 'string') || !Array.isArray(value.files) || !record(value.dependencies) || !Array.isArray(value.components)) throw new Error('components.json requires schemaVersion 1, a lowercase kebab-case name, files, dependencies, and components.');
