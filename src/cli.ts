...
import { addComponents, doctor, generateManifestCommand, infoComponent, listComponents, removeComponent, updateComponent, validateManifestCommand } from './commands.js';
...
export async function run(args: string[], cwd = process.cwd()): Promise<number> {
  ...
  try {
    ...
    else result = { output: 'Usage: ui components list|info, ui hooks list, ui add, ui update, ui remove, ui doctor, ui manifest validate|generate\n', exitCode: 1 };
