...
export async function run(args: string[], cwd = process.cwd()): Promise<number> {
  ...
  try {
    ...
    else if (positional[0] === 'component' && positional[1] === 'add') result = await addComponent(cwd, positional.slice(2), { dryRun: args.includes('--dry-run'), force: args.includes('--force'), update: false, json });
