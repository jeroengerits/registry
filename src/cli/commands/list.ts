...
export async function listComponent(cwd: string, json: boolean, showAvailableVersions = false): Promise<CommandResult> {
  ...
  return { output: json ? `${JSON.stringify(components, null, 2)}\n` : components.map((component) => `${component.name}@${component.version} (${component.path})${component.repository ? ` - ${colors.muted(component.repository)}` : ''}${showAvailableVersions ? ` [available: ${(component.availableVersions ?? []).join(', ')}]` : ''}`).join('\n') + (components.length ? '\n' : 'No installed components.\n'), exitCode: 0 };
}
