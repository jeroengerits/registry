...
import { addComponent, componentHelp, componentVersions, doctor, help, infoComponent, initProject, listComponent, outdatedComponents, removeComponent, revertComponent, selfUpdate, toggleComponent, updateComponent } from './index.js';
...
const componentDefinitions: CommandDefinition[] = [
  ...
  { path: 'component info', usage: '[name] [--json]', description: 'Show an installed component.', options: [jsonOption], register: (parent, cwd, setResult, definition) => { const command = configure(parent.command('info [name]'), definition); command.action(async (name: string | undefined, options: { json?: boolean }) => setResult(await infoComponent(cwd, name, Boolean(options.json)))); } },
   ...
   { path: 'component revert', description: 'Revert the last component update.', register: (parent, cwd, setResult) => { parent.command('revert').description('Revert the last component update.').action(async () => setResult(await revertComponent(cwd))); } },
  { path: 'component toggle', usage: '[name] [--json]', description: 'Toggle an installed component.', hidden: true, options: [jsonOption], register: (parent, cwd, setResult) => { const command = parent.command('toggle [name]', { hidden: true }); command.option(jsonOption.flags, jsonOption.description).action(async (name: string | undefined, options: { json?: boolean }) => setResult(await toggleComponent(cwd, name, Boolean(options.json)))); } },
   { path: 'component update', usage: '[name] [--version <version>] [--json]', description: 'Update to the latest compatible or specified version.', options: [{ flags: '--version <version>', description: 'Update to one stable version.' }, jsonOption], register: (parent, cwd, setResult) => { const command = parent.command('update [name]').description('Update to the latest compatible or specified version.'); command.option('--version <version>', 'Update to one stable version.').option(jsonOption.flags, jsonOption.description).action(async (name: string | undefined, options: { json?: boolean; version?: string }) => setResult(await updateComponent(cwd, name, Boolean(options.json), options.version))); } },
