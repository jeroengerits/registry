...
export function registerCommands(program: Command, cwd: string, setResult: (result: CommandResult) => void): void {
  ...
  program.command('add [repositories...]').description('Install a component. Use - to read newline-delimited sources from stdin.').option('--version <version>', 'Install a specific version.').option('--dry-run', 'Preview changes without writing files.').option('--force', 'Overwrite an installed component.').option('--json', 'Print machine-readable JSON.').action(async (repositories: string[], options: { version?: string; dryRun?: boolean; force?: boolean; json?: boolean }) => setResult(await addComponent(cwd, await expandSources(repositories ?? []), { dryRun: Boolean(options.dryRun), force: Boolean(options.force), update: false, version: options.version, json: Boolean(options.json) })));
  ...
  program.command('remove [name]').description('Remove an installed component.').option('--dry-run', 'Preview removal without changing files.').option('--yes', 'Confirm removal without prompting.').option('--json', 'Print machine-readable JSON.').action(async (name: string | undefined, options: { json?: boolean; yes?: boolean; dryRun?: boolean }) => setResult(await removeComponent(cwd, name, Boolean(options.json), Boolean(options.yes), Boolean(options.dryRun))));
