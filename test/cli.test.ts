...
describe('validation', () => {
  ...
  it('keeps published JSON schemas aligned with the ui.json contracts', async () => {
    const componentSchema = JSON.parse(await readFile(path.join(process.cwd(), 'schemas/ui.schema.json'), 'utf8'));
    const stateSchema = JSON.parse(await readFile(path.join(process.cwd(), 'schemas/registry.schema.json'), 'utf8'));
